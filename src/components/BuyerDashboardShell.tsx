...
import { LayoutDashboard, Store, ShoppingBag, ShoppingCart, User, ChevronRight } from "lucide-react";
...
/**
 * Persistent sidebar (desktop) / pill tab bar (mobile) + top bar shared by every
 * buyer-facing app page — Overview, Shop, My Orders, Cart, Profile — so the
 * dashboard chrome never disappears when navigating between them.
 */
export function BuyerDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        {/* -- Main content -- */}
        <main className="flex-1 overflow-y-auto bg-background pb-[calc(3.5rem+env(safe-area-inset-bottom))] lg:pb-0">{children}</main>
      </div>
